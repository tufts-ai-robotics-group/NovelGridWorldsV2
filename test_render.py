...
class TestRender:
    def setUp(self):
        ...
        pogoist: Entity = self.state.place_object(
            "agent", Entity, properties={"loc": (2, 2)}
        )
        pogoist.inventory = {}
        ...
        for i in range(6):
            ...
            self.state.place_object(
                "bedrock", PolycraftObject, properties={"loc": (i, 9)}
            )
        for i in range(4):
            self.state.place_object(
                "bedrock", PolycraftObject, properties={"loc": (5, 10 + i)}
            )
        self.state.remove_object("bedrock", (4, 9))
        self.state.place_object("door", Door, properties={"loc": (4, 9)})
        self.state.place_object("chest", Chest, properties={"loc": (2, 13)})

        cObj = self.state.get_object_at((2, 13))
        cObj.inventory = {"iron": 3}
        ...
        recipe_dict = {
            "recipes": {
                ...
                "plank": {"input": [{"tree": 1}], "output": [{"plank": 4}]},
                "pogo_stick": {
                    "input": [{"stick": 4}, {"plank": 2}, {"rubber": 1}],
                    "output": [{"pogo_stick": 1}],
                },
            }
        }
        ...
        self.actions = {
            ...
            "craft_plank": Craft(
                state=self.state, recipe=recipe_dict["recipes"]["plank"]
            ),
            "craft_pogo_stick": Craft(
                state=self.state, recipe=recipe_dict["recipes"]["pogo_stick"]
            ),
        }

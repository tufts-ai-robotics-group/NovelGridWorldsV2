...
from gym_novel_gridworlds2.contrib.polycraft.actions.use_door import UseDoor
...
class UseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.actions = {
            ...
            "use_door": UseDoor(state=self.state),
        }
    ...
    def testUseDoor(self):
        ...
        self.actions["use_door"].do_action(agent, obj)

        hbn = self.state.get_object_at((0,2))
        self.assertEqual(hbn.isOpen, True)

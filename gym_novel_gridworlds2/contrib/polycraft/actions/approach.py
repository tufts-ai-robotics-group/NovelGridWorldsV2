...
class Approach(Action):
    ...
    def _find_nearest_obj(self, 
        ...
        while not q.empty():
            ...
            for _, vec in search_directions.items():
                ...
                if not visited[new_loc] and self.state._map[new_loc] is None:
                    q.put(new_loc)
        ...
        **kwargs
        ...
        if target_object is None:
            ...
            if len(target_object_list) == 0:
                return False, None
            direction, nearest_loc = self._find_nearest_obj(agent_entity.loc, desired_type=target_type)
            return nearest_loc is None, direction, nearest_loc
        else:
            raise NotImplementedError("Approa")
    ...
    def do_action(self, agent_entity: Entity, target_type: str = None, target_object: Object = None, **kwargs):

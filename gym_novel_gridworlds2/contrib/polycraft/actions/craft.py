...
class Craft(Action):
    ...
    def check_precondition(self, agent_entity: Entity, target_type=None, target_object=None):
        ...
        for item in self.recipe["input"]:
            ...
            if temp[0] in agent_entity.inventory:
                    if self.recipe["input"][count][temp[0]] > agent_entity.inventory[temp[0]]: 
    ...
    def do_action(self, agent_entity: Entity, target_type=None, target_object=None):
        ...
        for item in self.recipe["input"]:
            temp = list(item.keys())
            agent_entity.inventory[temp[0]] = agent_entity.inventory[temp[0]] - self.recipe["input"][count][temp[0]]
            count += 1
        if self.itemToCraft in agent_entity.inventory:
            agent_entity.inventory[self.itemToCraft] += self.recipe["output"][0][self.itemToCraft]
        else:
            agent_entity.inventory[self.itemToCraft] = self.recipe["output"][0][self.itemToCraft]
